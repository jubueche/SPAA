...
if __name__ == "__main__":
    ...
    model_save_path = path.join(base_path, "BPTT_small_trained_martino.pth")
    ...
    epochs = FLAGS.epochs
    print(f"Batch size: {batch_size}, dt: {dt}, epochs: {epochs}")

    ibm_gesture_dataloader = IBMGesturesDataLoader()

    data_loader_train = ibm_gesture_dataloader.get_data_loader(
        "train", shuffle=True, num_workers=4, batch_size=batch_size, dt=dt, force_n_bins=(tot_length*1000)//dt)
    data_loader_test = ibm_gesture_dataloader.get_data_loader(
        "test", shuffle=True, num_workers=4, batch_size=batch_size//2, dt=dt)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (sample, target) in enumerate(pbar):
            model.reset_states()
            sample = sample.float()
