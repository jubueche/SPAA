...
plt.subplot(186)
plt.imshow(torch.squeeze(round_fn(X_adv_sparse_fool).cpu())); plt.title(f"Adv. SparseFool: {int(model_pred_sparsefool)}")
plt.subplot(187)
plt.imshow(torch.squeeze(torch.round(reparameterization_bernoulli(X_adv_sparse_fool_prob, temperature=0.01)).cpu())); plt.title(f"Adv. SparseFool Prob: {int(model_pred_sparsefool_prob)}")
plt.show()